...
st.set_page_config(page_title="DTEN Linkage (Final Match)", layout="wide")
st.title("🔥 DTEN Linkage Processor (Match Final Sheet)")
...
if file_req and file_res:
    ...
    # Extract Request
    # =========================
    extracted = df_req.apply(extract_req_row, axis=1)
    extracted.columns = ["deviceId", "RequestId"]

    df_req["deviceId"] = extracted["deviceId"]
    df_req["RequestId"] = extracted["RequestId"]
    ...
    # Show
    # =========================
    st.success("✅ DTEN Linkage = MATCH FINAL")
